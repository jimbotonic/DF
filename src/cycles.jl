...
"""
    list_colinks(g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}},rg::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}},filename::String) where {T<:Unsigned}
...
"""
    list_colinks(g::LightGraphs.AbstractGraph{T},rg::LightGraphs.AbstractGraph{T},filename::String) where {T<:Unsigned}

Get the list of colinks
"""
function list_colinks(g::LightGraphs.AbstractGraph{T},rg::LightGraphs.AbstractGraph{T},filename::String) where {T<:Unsigned}
	vs = vertices(g)
	n = nv(g)
	io = T[]
	# number of unique colinks per node
...
"""
    list_triangles_map(v::T,g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}},rg::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}},dpos::Dict{T,T}) where {T<:Unsigned}
...
"""
    list_triangles_map(v::T,g::LightGraphs.AbstractGraph{T},rg::LightGraphs.AbstractGraph{T},dpos::Dict{T,T}) where {T<:Unsigned}

	List triangles - map
"""
function list_triangles_map(v::T,g::LightGraphs.AbstractGraph{T},rg::LightGraphs.AbstractGraph{T},dpos::Dict{T,T}) where {T<:Unsigned}
	candidates = (T,T,T)[]
	gc = filter(x->dpos[x]>dpos[v],outneighbors(v,g))
	gp = filter(x->dpos[x]>dpos[v],outneighbors(v,rg))
	#@debug("# greater children: ", length(gc))
	#@debug("# greater parents: ", length(gp))
	for c in gc
		# rank(child) > rank(v)
...
"""
    list_triangles_reduce(candidates::Array{Tuple{T,T,T},1},g::LightGraphs.AbstractGraph{T},io::Array{T,1}) where {T<:Unsigned}

list triangles - reduce
"""
function list_triangles_reduce(candidates::Array{Tuple{T,T,T},1},g::LightGraphs.AbstractGraph{T},io::Array{T,1}) where {T<:Unsigned}
	count = convert(T,0)
	for c in candidates
		cc = outneighbors(convert(T,c[2]),g)
		sort!(cc)
		if binary_search(cc,c[3]) != -1
			append!(io,[c[1],c[2],c[3]])
			count = convert(T,1+count)
...
"""
    list_triangles_mapreduce(v::T,g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}},rg::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}},dpos::Dict{T,T},io::Array{T,1}) where {T<:Unsigned}
...
"""
    list_triangles_mapreduce(v::T,g::LightGraphs.AbstractGraph{T},rg::LightGraphs.AbstractGraph{T},dpos::Dict{T,T},io::Array{T,1}) where {T<:Unsigned}

List triangles - map&reduce
"""
function list_triangles_mapreduce(v::T,g::LightGraphs.AbstractGraph{T},rg::LightGraphs.AbstractGraph{T},dpos::Dict{T,T},io::Array{T,1}) where {T<:Unsigned}
	count = convert(T,0)
	vpos = dpos[v]
	gc = filter(x->dpos[x]>vpos,outneighbors(v,g))
	gp = filter(x->dpos[x]>vpos,outneighbors(v,rg))
	for c in gc
		cc = outneighbors(c,g)
		sort!(cc)
		for p in gp
...
"""
    init_un23_arrays(g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}},rg::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}},dpos::Dict{T,T}) where {T<:Unsigned}
...
"""
    init_un23_arrays(g::LightGraphs.AbstractGraph{T},rg::LightGraphs.AbstractGraph{T},dpos::Dict{T,T}) where {T<:Unsigned}

Initialize un2 (# of colink candidates) and un3 (# of triangle candidates) array
"""
function init_un23_arrays(g::LightGraphs.AbstractGraph{T},rg::LightGraphs.AbstractGraph{T},dpos::Dict{T,T}) where {T<:Unsigned}
	@debug("initializing # colink candidates + # triangle candidates arrays")
	vs = vertices(g)
	n = nv(g)
	un2 = Uint64[]
	...
	for v in vs
		# total order relation 1
		# get the set of greater children of vertex v
		gc1 = filter(x->x>v,outneighbors(v,g))
		push!(un2, length(gc1))

		# total order relation 2
		vpos = dpos[v]
		gc2 = filter(x->dpos[x]>vpos,outneighbors(v,g))
		gp = filter(x->dpos[x]>vpos,outneighbors(v,rg))
		p = length(gp)
		c = length(gc2)
		i = length(intersect(gp,gc2))
...
"""
    init_mn23_arrays(g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}},rg::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}}) where {T<:Unsigned}
...
"""
    init_mn23_arrays(g::LightGraphs.AbstractGraph{T},rg::LightGraphs.AbstractGraph{T}) where {T<:Unsigned}

Initialize mn2 (maximum # of colinks) and mn3 (maximum # of triangles) array
"""
function init_mn23_arrays(g::LightGraphs.AbstractGraph{T},rg::LightGraphs.AbstractGraph{T}) where {T<:Unsigned}
	@debug("initializing max # colinks + max # triangles arrays")
	vs = vertices(g)
	n = nv(g)
	mn2 = Uint64[]
...
"""
    list_triangles(g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}},rg::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}},filename::String) where {T<:Unsigned}
...
"""
    list_triangles(g::LightGraphs.AbstractGraph{T},rg::LightGraphs.AbstractGraph{T},filename::String) where {T<:Unsigned}

Get the list of triangles
"""
function list_triangles(g::LightGraphs.AbstractGraph{T},rg::LightGraphs.AbstractGraph{T},filename::String) where {T<:Unsigned}
	n = nv(g)
	io = T[]
	# actual number of unique triangles
	un3 = zeros(Uint64,n)
