...
""" 
    RW(g::LG.AbstractGraph{T},n_steps::UInt64,starting_v::T=convert(T,1)) where {T<:Unsigned}
...
@return the list of visited nodes
"""
function RW(g::LG.AbstractGraph{T},n_steps::UInt64,starting_v::T=convert(T,1)) where {T<:Unsigned}
	visited_nodes =  T[]
	v = starting_v
	for i in 1:n_steps
...
"""
    RW_aggregated(g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}},jumping_constant::Float64,starting_v::T=convert(T,1)) where {T<:Unsigned}
...
"""
    RW_aggregated(g::LG.AbstractGraph{T},jumping_constant::Float64,starting_v::T=convert(T,1)) where {T<:Unsigned}
...
@return an array vid position -> # visits
"""
function RW_aggregated(g::LG.AbstractGraph{T},jumping_constant::Float64,starting_v::T=convert(T,1)) where {T<:Unsigned}
...
""" 
    RW_aggregated(g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}},n_steps::UInt64,starting_v::T=convert(T,1)) where {T<:Unsigned}
...
""" 
    RW_aggregated(g::LG.AbstractGraph{T},n_steps::UInt64,starting_v::T=convert(T,1)) where {T<:Unsigned}
...
@return an array vid position -> # visits
"""
function RW_aggregated(g::LG.AbstractGraph{T},n_steps::UInt64,starting_v::T=convert(T,1)) where {T<:Unsigned}
	vv =  zeros(UInt32,nv(g))
	v = starting_v
	vv[v] = 1
	for i in 1:n_steps
...
""" 
    US(g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}},n_steps::UInt64) where {T<:Unsigned}
...
""" 
    US(g::LG.AbstractGraph{T},n_steps::UInt64) where {T<:Unsigned}
...
@return the list of sampled vertices
"""
function US(g::LG.AbstractGraph{T},n_steps::UInt64) where {T<:Unsigned}
	visited_nodes =  T[]
	n = nv(g)
	for i in 1:n_steps
		pv = convert(T,rand(1:n))
...
"""
    ARW(g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}}, n_steps::UInt64, rd::Array{Float64,1}, starting_v::T=convert(T,1)) where {T<:Unsigned}
...
"""
    ARW(g::LG.AbstractGraph{T},n_steps::UInt64,rd::Array{Float64,1},starting_v::T=convert(T,1)) where {T<:Unsigned}
...
@return an array vid position -> # visits
"""
function ARW(g::LG.AbstractGraph{T},n_steps::UInt64,rd::Array{Float64,1},starting_v::T=convert(T,1)) where {T<:Unsigned}
	visited_nodes =  T[]
	v = starting_v
	push!(visited_nodes,v)
	rejected = 0
...
""" 
    ARW_flying(g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}}, n_steps::UInt64, rd::Array{Float64,1}, starting_v::T=convert(T,1)) where {T<:Unsigned}
...
""" 
    ARW_flying(g::LG.AbstractGraph{T},n_steps::UInt64,rd::Array{Float64,1},starting_v::T=convert(T,1)) where {T<:Unsigned}
...
@return an array vid position -> # visits
"""
function ARW_flying(g::LG.AbstractGraph{T},n_steps::UInt64,rd::Array{Float64,1},starting_v::T=convert(T,1)) where {T<:Unsigned}
	visited_nodes =  T[]
	v = starting_v
	push!(visited_nodes,v)
	for i in 1:n_steps
...
""" 
    MHRW(g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}}, n_steps::UInt64, in_degrees::Array{T,1}, out_degrees::Array{T,1},starting_v::T=convert(T,1)) where {T<:Unsigned}
...
""" 
    MHRW(g::LG.AbstractGraph{T},n_steps::UInt64,in_degrees::Array{T,1},out_degrees::Array{T,1},starting_v::T=convert(T,1)) where {T<:Unsigned}
...
@return an array vid position -> # visits
"""
function MHRW(g::LG.AbstractGraph{T},n_steps::UInt64,in_degrees::Array{T,1},out_degrees::Array{T,1},starting_v::T=convert(T,1)) where {T<:Unsigned}
	visited_nodes =  T[]
	v = starting_v
	push!(visited_nodes,v)
	rejected = 0
...
"""
    MHRW_flying(g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}}, n_steps::UInt64, in_degrees::Array{T,1}, out_degrees::Array{T,1},starting_v::T=convert(T,1)) where {T<:Unsigned}
...
"""
    MHRW_flying(g::LG.AbstractGraph{T},n_steps::UInt64,in_degrees::Array{T,1},out_degrees::Array{T,1},starting_v::T=convert(T,1)) where {T<:Unsigned}

Metropolis-Hasting Random Walk (flying mode)
@return an array vid position -> # visits
"""
function MHRW_flying(g::LG.AbstractGraph{T},n_steps::UInt64,in_degrees::Array{T,1},out_degrees::Array{T,1},starting_v::T=convert(T,1)) where {T<:Unsigned}
	visited_nodes =  T[]
	v = starting_v
	push!(visited_nodes,v)
	for i in 1:n_steps
...
"""
    CC_MHRW_flying(g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}}, n_steps::UInt64, ccs::Array{Float64,1}, starting_v::T=convert(T,1)) where {T<:Unsigned}
...
"""
    CC_MHRW_flying(g::LG.AbstractGraph{T},n_steps::UInt64,ccs::Array{Float64,1},starting_v::T=convert(T,1)) where {T<:Unsigned}

RW in flying mode guided by the colink (C1C) or clustering (C2C) coefficient

Twitter dataset: nchains: 100, burning_time: 100, nsteps: 5000
"""
function CC_MHRW_flying(g::LG.AbstractGraph{T},n_steps::UInt64,ccs::Array{Float64,1},starting_v::T=convert(T,1)) where {T<:Unsigned}
	visited_nodes =  T[]
	v = starting_v
	push!(visited_nodes,v)
	visited_distr = Dict{T,T}()
...
""" 
    get_CC_MHRW_flying_ball(g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}},n_vertices::T,ccs::Array{Float64,1},starting_v::T=convert(T,1),jumping_constant::Float64=0.) where {T<:Unsigned}
...
""" 
    get_CC_MHRW_flying_ball(g::LG.AbstractGraph{T},n_vertices::T,ccs::Array{Float64,1},starting_v::T=convert(T,1),jumping_constant::Float64=0.) where {T<:Unsigned}

Explore a ball centered around starting vertex with a modified random walk
"""
function get_CC_MHRW_flying_ball(g::LG.AbstractGraph{T},n_vertices::T,ccs::Array{Float64,1},starting_v::T=convert(T,1),jumping_constant::Float64=0.) where {T<:Unsigned}
	visited_nodes =  Set{T}()
	v = starting_v
	push!(visited_nodes, v)
	while length(visited_nodes) < n_vertices
