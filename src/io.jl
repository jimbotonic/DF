...
using Graphs, LightGraphs, DataStructures, HDF5, JLD
...
""" 
    load_mgs1_graph(g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}},name::AbstractString) where {T<:Unsigned}
...
"""
    write_mgs3_graph(g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}}, filename::AbstractString) where {T<:Unsigned}
...
"""
    write_mgs3_graph(g::LG.AbstractGraph{T}, filename::AbstractString) where {T<:Unsigned}

load graph in format MGS v3
"""
function write_mgs3_graph(g::LG.AbstractGraph{T}, filename::AbstractString) where {T<:Unsigned}
  	# Header 12 bytes: 
	# -> version: 7 bytes string
	# -> size T: 1 byte
	# -> # vertices: 4 bytes position 
...
""" 
    load_mgs3_graph(g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}}, filename::AbstractString) where {T<:Unsigned}
...
""" 
    load_mgs3_graph(g::LG.AbstractGraph{T}, filename::AbstractString) where {T<:Unsigned}

load graph in format MGS v3
"""
function load_mgs3_graph(g::LG.AbstractGraph{T}, filename::AbstractString) where {T<:Unsigned}
	f = open(filename, "r")
	### read header
	# 7-bytes version
	version = read(f,7)
...
""" 
    load_mgs4_graph(g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}}, filename::AbstractString) where {T<:Unsigned}
...
""" 
    load_mgs4_graph(g::LG.AbstractGraph{T}, filename::AbstractString) where {T<:Unsigned}

load graph in format MGS v4
"""
function load_mgs4_graph(g::LG.AbstractGraph{T}, filename::AbstractString) where {T<:Unsigned}
	f = open(filename, "r")
	### read header
  	# 8 bytes: 7 bytes string + 1 byte: 'MGSv4  ' + <8bits T size>
	# 4 bytes (32bits): offset in size_t of graph data section (i.e. # of vertices)
...
""" 
    load_adjacency_list_from_csv(::Type{T}, g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}}, filename::AbstractString, separator::Char=' ') where {T<:Unsigned}
...
function load_adjacency_list_from_csv(::Type{T}, g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}}, filename::AbstractString, separator::Char=' ') where {T<:Unsigned}
	...
	for i in 1:length(edges)
		egde = edges[i]
		G.add_edge!(g, edges[i][1], edges[i][2])	
	end
end

""" 
    load_adjacency_list_from_csv(::Type{T}, g::LG.AbstractGraph{T}, filename::AbstractString, separator::Char=' ') where {T<:Unsigned}

load graph from CSV adjacency list
"""
function load_adjacency_list_from_csv(::Type{T}, g::LG.AbstractGraph{T}, filename::AbstractString, separator::Char=' ') where {T<:Unsigned}
	f = open(filename,"r")
	oni = Dict{T,T}()
	edges = Array{Tuple{T,T},1}()
	counter = convert(T,1)
	...
	# add vertices
	LG.add_vertices!(g,length(keys(oni)))
	
	# add edges
	for i in 1:length(edges)
		egde = edges[i]
		LG.add_edge!(g, edges[i][1], edges[i][2])	
	end
end

""" 
    load_graph_from_pajek(::Type{T}, g::GenericAdjacencyList{T,Array{T,1},Array{Array{T,1},1}}, filename::AbstractString) where {T<:Unsigned}
	...
	while !eof(f)
		...
		if !startswith(line, "%")
			...
			if inside_vertices_section
				sa = split(line, ' ')
        			G.add_vertex!(g, parse(T,sa[1]))
			elseif inside_edges_section
				sa = split(line, ' ')
				G.add_edge!(g, parse(T, sa[1]), parse(T,sa[2]))
			end
		end
	end
	close(f)
